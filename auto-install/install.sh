...
whiptail --msgbox --backtitle "Welcome" --title "Automated Installer" "This installer will transform your Raspberry Pi into a FixMix.  NOTE: This installer is intended to be run on a fresh install of Raspbian Lite Stretch.  This script is currently in testing so there may be bugs." ${r} ${c}
...
echo "*************************************************************************"
cd /usr/local/bin
$SUDO git clone https://github.com/schnigglas/fixmix
...
cd /usr/local/bin/fixmix/auto-install/nginx

# Delete default configuration
$SUDO rm /etc/nginx/sites-enabled/default

# Copy configuration file to nginx
$SUDO cp proxy.nginx /etc/nginx/sites-available/fixmix

# Create link in sites-enabled
$SUDO ln -s /etc/nginx/sites-available/fixmix /etc/nginx/sites-enabled
...
# cd /usr/local/bin/usr/local/bin/fixmix/auto-install/supervisor

$SUDO cp /usr/local/bin/fixmix/auto-install/supervisor/*.conf /etc/supervisor/conf.d/
